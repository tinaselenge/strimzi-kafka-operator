...
class KafkaQuorumCheck2 {
    ...
    protected KafkaQuorumCheck2(Reconciliation reconciliation, Admin ac) {
        this.reconciliation = reconciliation;
        this.admin = ac; //TODO: should this be the abstract client instead?
    }
    ...
    CompletableFuture<Boolean> canRollController(int nodeId) {
        ...
        return describeMetadataQuorum().whenComplete((qrmInfo, error) -> {
            if (error != null) {
                LOGGER.warnCr(reconciliation, "Error determining whether it is safe to restart controller pod {}", nodeId, unwrap(error));
            }
        }).thenApply(info -> {
    ...
    private long getControllerQuorumFetchTimeoutMs(int leaderId) {
        ...
        try {
            ConfigResource resource = new ConfigResource(ConfigResource.Type.BROKER, String.valueOf(leaderId));
            var result = admin.describeConfigs(Collections.singleton(resource)).values();
            if (result.containsKey(resource)) {
                var config = result.get(resource).get();
                if (config != null) {
    ...
    private CompletableFuture<QuorumInfo> describeMetadataQuorum() {
        CompletableFuture<QuorumInfo> cf = new CompletableFuture<>();
        admin.describeMetadataQuorum().quorumInfo().whenComplete((quorumInfo, throwable) -> {
            if (throwable != null) {
                cf.completeExceptionally(throwable);
            } else {
                cf.complete(quorumInfo);
            }
        });
        return cf;
    }
